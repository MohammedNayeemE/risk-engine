...
from agents.generalised_etl.prompts import (
    age_restriction,
    agentic_chunking,
    dosage,
    identity,
)
from agents.generalised_etl.prompts import regulatory as regulatory_prompt
from agents.generalised_etl.prompts import strength
...
llama_model = ChatOllama(model="llama3:latest", temperature=0.1)
...
def _extract_with_llm(human_content: str, system_content: str, structure) -> dict:
    response = llama_model.with_structured_output(structure).invoke(
